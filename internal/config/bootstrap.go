...
func EnsureConfigFile(path string) (*InitialAdminCredentials, error) {
	...
	startup := ensureMapping(root, "startup")
	setMapInt(startup, "worker_bootstrap_concurrency", DefaultConfig().Startup.WorkerBootstrapConcurrency)
	setMapInt(startup, "state_sync_concurrency", DefaultConfig().Startup.StateSyncConcurrency)
