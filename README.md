...
正常数据承载每 10 分钟检查一次公网 IPv4/IPv6。同一连接周期连续 6 次探测失败（包括停机 SIM 长期没有承载地址）后，VoHive 会进入降频状态，改为标称每 2 小时检查一次，并按设备加入稳定的 ±10% 抖动，避免多卡同时请求和重复告警刷屏。

未发生地址变化的健康轮询和 QMI/MBIM 事件不会绕过已排定的重试；数据承载真实重连、私网地址或地址族变化、Worker 替换以及 eSIM 轮换会开始新的探测周期。降频状态只保存在内存中，服务重启后会重新执行一轮快速确认。
