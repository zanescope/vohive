...
上例列出中国大陆网络常用的第三方双栈 HTTPS 回显端点，但不承诺其可用性、限流策略或长期兼容性；生产环境建议优先使用受信任的自建端点。HTTPS 探测依赖系统 CA 根证书；官方容器镜像已内置证书，OpenWrt 或直接运行发布二进制时需安装 `ca-bundle` 或系统等价的 `ca-certificates`，不能关闭 TLS 校验规避错误。
